...
function PreviewCard({ selectedData }: PreviewCardProps) {
  ...
  const imageUrl =
    selectedData?.content_files?.[0]?.signed_url ??
  ...
  return (
    <div className="preview-container">
      <Grid>
        ...
        <Grid.Col span={3} className="image-column">
          <SchemaTable schema={selectedData?.schema}></SchemaTable>
        </Grid.Col>
        <Grid.Col span={3} style={{ position: "relative", padding: "10px" }}>
          {selectedData ? (
            <>
              <PreviewDataDiv
                name={"File Name"}
                val={getFileNameWithoutExtension(
...
export const SchemaTable = ({ schema }: SchemaTableProps) => {
  ...
  */
  const schemaData = schema
    ? Object.entries(schema).map(([name, value]) => ({ name, value }))
    : [];
  const [searchTerm, setSearchTerm] = useState("");
  const [filteredData, setFilteredData] = useState(schemaData);
  useEffect(() => {
    setFilteredData(schemaData);
  }, [schema]); // Dependency array ensures this runs when schema changes

  // Function to filter data based on the search term
  const handleSearch = (term: string) => {
    const lowercasedTerm = term.toLowerCase();
    const filtered = schemaData?.filter(
      (item) =>
